export const SAMPLE_DATA_PAGE = {
  addSecurityInformationDataButton:
    'div:nth-child(1) > div > div.euiCard__footer > div > div > button',
  addAuditingAndPolicyMonitoringDataButton:
    'div:nth-child(2) > div > div.euiCard__footer > div > div > button',
  addThreatDetectionAndResponseDataButton:
    'div:nth-child(3) > div > div.euiCard__footer > div > div > button',
  dataAddedSuccessfullyToast: '.euiCard__betaBadgeWrapper',
};
